...
**Goal:** Every category of personal data terminates on infrastructure the user controls (device or homelab), reachable over open protocols, with independent encrypted backups — no Google account as the root of trust for anything.

Four layers:

| Layer | What lives here |
|---|---|
| **Device** | GrapheneOS, sandboxed Google Play Services (compatibility only), local-first apps |
| **Sync/Protocol** | CardDAV, CalDAV, SMB — open protocols, no vendor API in the path |
| **Self-Hosted Services** | Homelab/NAS: Immich, Bitwarden, Traccar, Jellyfin, Home Assistant |
...
**Why this works:** encryption happens as close to the data's origin as possible (client-side for Bitwarden and Cryptomator), so plaintext crosses the fewest possible trust boundaries. Every row has ≥2 independent backup paths for Tier-0/Tier-1 data — no single tool failure is fatal.
...
| 004 | HERE WeGo | Google Maps | Offline maps, no account linkage | Still a 3rd-party map data source |
| 005 | Immich | Google Photos | Local ML (face/object detection), no upload target | Needs real compute/storage; fast-moving project |
| 006 | Synology Photos | Google Photos (alt.) | NAS-native integration, on-prem | Closed-source app layer |
| 007 | **Bitwarden (self-hosted)** | Google Password Manager **+ Google Authenticator** | Zero-knowledge vault **and** built-in TOTP generator in one encrypted store — single root of trust instead of two | Operator owns patching/uptime of the vault server |
| 008 | NewPipe | YouTube app | No embedded SDKs, no account binding | Depends on reverse-engineered API |
| 009 | Traccar | Google Location History | Self-hosted, user-defined retention | Needs an always-reachable endpoint |
| 010 | SMBSync3 | Google Drive Sync | LAN-scoped, no cloud intermediary | No off-network access without VPN |
| 011 | Jellyfin | YouTube Music | Open-source, no telemetry | Manual library curation |
| 012 | Home Assistant | Google Home | Local automation execution | Some devices still need cloud round-trips |
| 013 | Restic | — | Client-side encrypted, deduplicated, verifiable | CLI-driven; repo password loss = unrecoverable |
| 014 | Duplicati | — (secondary to Restic) | Independent key material, GUI recovery path | Historically less stable; never the sole backup |
| 015 | Cryptomator | Google Drive (sensitive files) | Encrypts before data reaches any sync/storage layer | Adds manual mount/unmount step |

---

## 6. Credential & 2FA Architecture (Bitwarden)

**Merged design:** passwords and TOTP/2FA secrets live in **one self-hosted Bitwarden vault** instead of two separate tools (password manager + Aegis). Bitwarden's built-in authenticator field stores the TOTP seed alongside the credential it protects, and autofill can present the generated code inline.
...
**Trade-off:** storing TOTP seeds in the same vault as the passwords they protect narrows the separation-of-secrets model — a full vault compromise now exposes both factors together. This is accepted in exchange for operational simplicity; sensitive administrative accounts can still use a hardware key (FIDO2) as a second, vault-independent factor where extra separation is warranted.
...
| Tier | Assets | Why |
|---|---|---|
| **Tier-0** | Bitwarden vault (passwords + TOTP), encryption keys, Cryptomator recovery material | Non-regenerable and gating — losing them can lock out every other service |
| **Tier-1** | Contacts, calendars, Home Assistant data, Traccar data | Important, but recreatable/re-syncable |
| **Tier-2** | Photos, videos, music, voice recordings | Recoverable from other copies/devices |

```mermaid
flowchart LR
    User((User)) --> GOS[GrapheneOS Device]
    GOS --> LocalApps[Local-First Apps]
...
Every arrow terminates in user-owned infrastructure or a client-side encrypted boundary — no primary path includes a third-party cloud hop.
...
|---|---|
| Encrypted at every hop | Vault exports and Cryptomator layer are encrypted before touching sync/storage |
...
**Why the merge simplifies recovery:** previously, losing 2FA access required a separate Aegis-export recovery procedure that could leave Bitwarden itself locked out. With TOTP inside Bitwarden, one successful vault restore unlocks everything downstream (Home Assistant, Traccar, server admin) in a single step.
...
| Excessive permissions | Broad bundled-app permissions | GrapheneOS per-app network/permission scoping |
...
flowchart LR
    subgraph Traditional["Google Ecosystem"]
        T1[Mass Surveillance]; T2[Ad Tracking]; T3[Vendor Lock-In]; T4[Cloud Dependency]
    end
    subgraph SelfHostedArch["This Architecture"]
        S1[Fragmented, Local-First Data]; S2[No Ad-Funded Services]; S3[Standards-Based Portability]; S4[Independent Local Services]
    end
    T1 -.reduced by.-> S1
    T2 -.reduced by.-> S2
    T3 -.reduced by.-> S3
...
| Isolation | Scoped per profile, network access toggled per app |
...
| Network segmentation (VLANs) | Flat homelab network | Limits lateral movement if one service is compromised |
